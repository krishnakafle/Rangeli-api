...
#     SELECT NULL AS id, ST_AsMVT(q, 'default', 4096, 'mvt_geom')
#         FROM (SELECT {select_statement}
#             ST_AsMVTGeom(ST_Transform(join_f.{self.geo_col}, 3857),
#             ST_Transform(ST_SetSRID(ST_GeomFromText(%s), 4326), 3857), 4096, 0, false) AS mvt_geom
#         FROM (
#             SELECT household."{table}".*, public.municipality_house.geom
#                 FROM household."{table}", public.municipality_house
#                 Where public.municipality_house."houseId"=household."{table}".house_id
#         ) AS join_f
#         WHERE {parameterized_where_clause}
#         LIMIT %s
#         OFFSET %s) AS q;
...
app_label_matching_schema ={
    'genericHouseholdApp': 'household',
    'genericIndividualApp': 'individual',
    'genericMicellanousApp': 'micellanous',
    'genericBusinessApp': 'business',
    'genericAbsenteeApp': 'absentee',
    'genericLandApp': 'land',
    'genericLivestockApp': 'livestock',
    'institutionApp': 'institution',
...
class MVTManager(models.Manager):
    ...
    def _build_query(self, filters={}):
        ...
        try:
            schema = app_label_matching_schema[appLabel]
        except :
            schema = 'public'
        select_statement = self._create_select_statement(table, schema)
        (
            parameterized_where_clause,
            where_clause_parameters,
        ) = self._create_where_clause_with_params(table, filters, schema)
        query = f"""
        SELECT NULL AS id, ST_AsMVT(q, 'default', 4096, 'mvt_geom')
            FROM (SELECT {select_statement}
                ST_AsMVTGeom(ST_Transform({schema}."{table}".{self.geo_col}, 3857),
                ST_Transform(ST_SetSRID(ST_GeomFromText(%s), 4326), 3857), 4096, 0, false) AS mvt_geom
            FROM {schema}."{table}"
            WHERE {parameterized_where_clause}
            LIMIT %s
    ...
    def _create_where_clause_with_params(self, table, filters, schema):
        """
        Args:
        ...
        extra_wheres = " AND " + schema +'.' if params else ""
        extra_wheres += sql.split("WHERE")[1].strip() if params else ""
        where_clause = (
            f'ST_Intersects({schema}."{table}".{self.geo_col}, '
            f"ST_SetSRID(ST_GeomFromText(%s), 4326)){extra_wheres}"
        )
        # print(extra_wheres)
        return where_clause, list(params)

    def _create_select_statement(self, table, schema):
        """
        ...
        select_sql = (sql.split("FROM")[0].lstrip("SELECT ").strip() + ",").replace(table, table_with_schema)
        return select_sql
