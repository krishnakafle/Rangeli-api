...
class ExistingEmailFound(exceptions.APIException):
    status_code = 401
    default_detail = 'E-mail already registered.'


class ExistingUsernameFound(exceptions.APIException):
    status_code = 401
    default_detail = 'UserName already registered.'
...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if user_obj.is_active == False:
            ...
            email_subject = "RI Studio %s: Activate your accounts" % ("Panchkhal")
            ...
            context = {
                ...
                'portal_url': settings.FRONTEND_URL,
                'municipality': "Panchkhal",
                'link': "%sapi/accounts/activate/%s" % (settings.FRONTEND_URL, activation_key),
                'bcolor': '#00AC5B'
            }
            ...
            _email = EmailMessage(email_subject, html_content, to=[email])
            _email.content_subtype = "html"
            _email.send()

            # mail = MailGun()
            # sent = mail.send_mail(email, email_subject, False, html_content)
            # if sent:
            #     print('ok')
...
class MailGun:
    def __init__(self):
        ...
        self.host = 'https://api.mailgun.net/v3/{}/messages'.format(settings.EMAIL_HOST_USER)

    def send_mail(self, recipients, subject, message=False, html=False, attachment=False, bcc=False, form=settings.EMAIL_HOST_USER):
        ...
        r = EmailMessage(subject, html, to=[recipients]).send()
...
class UserNotActivated(exceptions.APIException):
    status_code = 401
    default_detail = 'User is not Activated.'


class EmailNotFound(exceptions.APIException):
    status_code = 401
    default_detail = 'E-mail Not Found.'
